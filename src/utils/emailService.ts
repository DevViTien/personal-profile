...
export const validateFormData = (data: ContactFormData): string | null => {
  ...
  const spamKeywords = ['viagra', 'casino', 'lottery', 'winner', 'congratulations', 'million', 'inheritance'];
...
export const sanitizeFormData = (data: ContactFormData): ContactFormData => {
  ...
  const removeScriptTags = (str: string) => str.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '');
  ...
  return {
    ...
    message: removeScriptTags(removeSpecialChars(data.message.trim())).substring(0, 2000),
  };
};
...
export const sendContactEmail = async (formData: ContactFormData): Promise<EmailResult> => {
  try {
    ...
    if (configError) {
      ...
      return {
        success: false,
        message: 'Dịch vụ email chưa được cấu hình đúng. Vui lòng liên hệ quản trị viên.',
      };
    }
    ...
    if (error && typeof error === 'object' && 'message' in error) {
      ...
      if (errorObj.message?.includes('network')) {
        ...
        errorMessage = 'Không có quyền gửi email. Vui lòng liên hệ quản trị viên.';
      } else if (errorObj.status === 429) {
        errorMessage = 'Đã gửi quá nhiều tin nhắn. Vui lòng thử lại sau ít phút.';
      }
    }
...
export const checkRateLimit = (): { allowed: boolean; timeLeft?: number } => {
  ...
  try {
    ...
    if (data.attempts >= MAX_ATTEMPTS) {
      const timeLeft = Math.ceil((RATE_LIMIT_DURATION - (now - data.firstAttempt)) / 1000 / 60);
